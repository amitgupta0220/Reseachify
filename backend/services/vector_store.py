...
# def add_text_summaries_to_vectorstore(texts, text_summaries):
#     """
#     Adds summarized text chunks to the vector store and links them to the document store.
#     """
#     doc_ids = [str(uuid.uuid4()) for _ in texts]
#     try:
#         for i, summary in enumerate(text_summaries):
#             vectorstore.add_documents([Document(page_content=summary, metadata={id_key: doc_ids[i]})])
#             docstore.mset([(doc_ids[i], texts[i])])  # Link original text to the vector store
#     except Exception as e:
#         raise RuntimeError(f"Error adding text summaries to the vector store: {str(e)}")

# def add_table_summaries_to_vectorstore(tables, table_summaries):
#     """
#     Adds summarized tables to the vector store and links them to the document store.
#     """
#     table_ids = [str(uuid.uuid4()) for _ in tables]
#     try:
#         for i, summary in enumerate(table_summaries):
#             vectorstore.add_documents([Document(page_content=summary, metadata={id_key: table_ids[i]})])
#             docstore.mset([(table_ids[i], tables[i])])  # Link original table to the vector store
#     except Exception as e:
#         raise RuntimeError(f"Error adding table summaries to the vector store: {str(e)}")

# def add_image_summaries_to_vectorstore(images, image_summaries):
#     """
#     Adds summarized images to the vector store and links them to the document store.
#     """
#     img_ids = [str(uuid.uuid4()) for _ in images]
#     try:
#         for i, summary in enumerate(image_summaries):
#             vectorstore.add_documents([Document(page_content=summary, metadata={id_key: img_ids[i]})])
#             docstore.mset([(img_ids[i], images[i])])  # Link original image to the vector store
#     except Exception as e:
#         raise RuntimeError(f"Error adding image summaries to the vector store: {str(e)}")

# def add_to_vectorstore(texts, text_summaries, tables, table_summaries, images, image_summaries):
#     """
#     Adds all data (text, tables, images) and their summaries to the vector store.
#     """
#     try:
#         add_text_summaries_to_vectorstore(texts, text_summaries)
#         add_table_summaries_to_vectorstore(tables, table_summaries)
#         add_image_summaries_to_vectorstore(images, image_summaries)
#     except Exception as e:
#         raise RuntimeError(f"Error adding data to the vector store: {str(e)}")
